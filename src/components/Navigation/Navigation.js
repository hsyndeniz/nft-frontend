...
const Navigation = ({ darkMode }) => {
  ...
  const handleConnectWallet = async () => {
    ...
    if (typeof window.ethereum !== "undefined") {
      ...
      const options = {
        method: 'POST',
        url: 'http://localhost:5000/v1/auth/authenticate',
      ...
      axios.request(options).then(function (response) {
        ...
        const message = getMessage(address.toLowerCase(), data.user.nonce);
        
        const message2 = `I am signing my one-time nonce: ${data.user.nonce}`;

        const sign = await window.ethereum.request({
          method: 'personal_sign',
          params: [message2, address.toLowerCase()],
        });
        console.log(sign);
        
        const _options = {
          method: 'POST',
          url: 'http://localhost:5000/v1/auth/verify-signature',
          data: {
            address: address.toLowerCase(),
            signature: sign
          }
        ...
        axios.request(_options).then(function (_response) {
          console.log(_response.data);
          localStorage.setItem('user', _response.data);
